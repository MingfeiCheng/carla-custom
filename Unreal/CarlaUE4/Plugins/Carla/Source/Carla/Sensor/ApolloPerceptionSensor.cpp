...
void AApolloPerceptionSensor::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaSeconds)
{
  ...
  Box->GetOverlappingActors(DetectedActors, FCarlaActor::StaticClass());
  ...
  if (DetectedActors.Num() > 0){
    {      
      ...
      for(auto& It : DetectedActors){
        ...
        const carla::rpc::ActorId ApolloActorId = ActorView->GetActorId();
        FBoundingBox ActorBBox = UBoundingBoxCalculator::GetActorBoundingBox(It);
        const carla::geom::Location ActorBBoxLocation = carla::geom::Location(0.0f, 0.0f, 0.0f);
        const carla::geom::Vector3D ActorBBoxExtent = carla::geom::Vector3D(0.0f, 0.0f, 0.0f);
        const carla::geom::Rotation ActorBBoxRotation = carla::geom::Rotation(0.0f, 0.0f, 0.0f);
        const carla::geom::BoundingBox ApolloActorBBox = carla::geom::BoundingBox(ActorBBoxLocation, ActorBBoxExtent, ActorBBoxRotation);
        // FCarlaActor::ActorType ActorType = ActorView->GetActorType();
        std::string ApolloActorType = "unknown";
        ...
        carla::geom::Vector3D ApolloActorAcceleration = carla::geom::Vector3D(0.0f, 0.0f, 0.0f);
        carla::geom::Vector3D ApolloActorVelocity = carla::geom::Vector3D(0.0f, 0.0f, 0.0f);
        const carla::geom::Location ApolloActorLocation = carla::geom::Location(0.0f, 0.0f, 0.0f);
        const carla::geom::Rotation ApolloActorRotation = carla::geom::Rotation(0.0f, 0.0f, 0.0f);
